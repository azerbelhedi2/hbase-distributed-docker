...
do
	HADOOP_SLAVE="$HOST_PREFIX"-slave-$i
	docker run --name $HADOOP_SLAVE -h $HADOOP_SLAVE.$NETWORK_NAME --net=$NETWORK_NAME -p 218$i:2181 -p 160$(($i+1))0:16020 -itd "$IMG_NAME" 
	i=$(( $i + 1 ))
done
...
docker run --name $HADOOP_MASTER -h $HADOOP_MASTER.$NETWORK_NAME --net=$NETWORK_NAME \
		-p  8088:8088  -p 9770:50070 -p 9790:50090 \
		-p 16010:16010 \
		-itd "$IMG_NAME"
